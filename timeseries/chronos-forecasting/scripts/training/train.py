...
class ChronosDataset(IterableDataset, ShuffleMixin):
    ...
    def to_hf_format(self, entry: dict) -> dict:
        past_target = torch.tensor(entry["past_target"], dtype=torch.float32).unsqueeze(0)
        input_ids, attention_mask, scale = self.tokenizer.context_input_transform(
            past_target
        )
        future_target = torch.tensor(entry["future_target"], dtype=torch.float32).unsqueeze(0)
...
class ChronosTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        self.model.eval()
        for inputs in self.get_eval_dataloader(eval_dataset):
            inputs = self._prepare_inputs(inputs)
            with torch.no_grad():
        ...
        metrics = {
            f"{metric_key_prefix}_loss": total_loss / max(count, 1),
            f"{metric_key_prefix}_runtime": 0,
            f"{metric_key_prefix}_samples_per_second": 0,
            f"{metric_key_prefix}_steps_per_second": 0,
        }
...
def main(
    ...
    # Validation dataset
    eval_dataset = None
    log_on_main(f"validation_data_paths raw: {validation_data_paths}", logger)
    if validation_data_paths is not None:
        if isinstance(validation_data_paths, str):
            validation_data_paths = ast.literal_eval(validation_data_paths)
        elif not isinstance(validation_data_paths, list):
            validation_data_paths = [validation_data_paths]
        log_on_main(f"Loading {len(validation_data_paths)} validation datasets", logger)


    # eval_dataset = None
    # if validation_data_paths is not None:
    #     validation_data_paths = ast.literal_eval(validation_data_paths) if isinstance(validation_data_paths, str) else validation_data_paths
    #     log_on_main(f"Loading {len(validation_data_paths)} validation datasets", logger)
