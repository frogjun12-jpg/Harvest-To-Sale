...
csv_path = OUTPUT_DIR / f"llm_only_response_time_benchmark_{timestamp}.csv"
json_path = OUTPUT_DIR / f"llm_only_response_time_benchmark_{timestamp}.json"
...
for target in targets:
    for category, question in questions:
        for run in range(1, 6):
            payload = {
                "question": question,
                "user_id": f"llm_only_{target['label']}_{category}_{run}_{int(time.time())}",
            ...
            print(f"{target['label']} | {category} | run {run} | {elapsed:.3f}s | {status}")
...
def summarize(group_rows):
    ...
    if not ok:
        return {"count": 0, "avg_sec": None, "median_sec": None, "min_sec": None, "max_sec": None}
